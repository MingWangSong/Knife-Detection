...
from utils import *
#from networks_arth import *
from networks_m_arth_attn import *
import time 
...
def main():
    ...
    for img_name in os.listdir(opt.data_path):
        if is_image(img_name):
            ...
            ret3, th3 = cv2.threshold(blur, 235, 255, cv2.THRESH_BINARY )
            
            b, g, r = cv2.split(y)
            b1, g1, r1 = cv2.split(th3)            
            y = cv2.merge([r, g, b])
            th3 = cv2.merge([r1, g1, b1])            
            ...
            th3 = normalize(np.float32(th3))
            th3 = np.expand_dims(th3.transpose(2, 0, 1), 0)
            th3 = Variable(torch.Tensor(th3))
