...
from networks_m_ed_attnv2 import *
...
parser.add_argument("--milestone", type=int, default=[40,60,80], help="When to decay learning rate")
...
#            loss = pixel_metric
        ...
        SSIMTotal = 0    
        count = 0
        for img_name in os.listdir("datasets/test/Rain100L_crop/rainy"):
            if is_image(img_name):
#                import pdb;pdb.set_trace()
                img_path = os.path.join("datasets/test/Rain100L_crop", "no"+img_name)
                img_path_rain = os.path.join("datasets/test/Rain100L_crop/rainy", img_name)
                # input image
                y = cv2.imread(img_path)
                y_noclean = cv2.imread(img_path_rain)	            
                b, g, r = cv2.split(y)
